...
int main(void){
    ...
    for(i = 0; i < iV; i++){
        for(j = 0; j < iV; j++) minArr[j] = W_MAX; // w <= 10
        curNode = root[i];
        while(curNode->next){   // scan min value
            if(curNode->next->len < minArr[curNode->next->dst]) {
                minArr[curNode->next->dst] = curNode->next->len;
            }
            curNode = curNode->next;
        }
        curNode = root[i];
        while(curNode && curNode->next){
            if(curNode->next->len == minArr[curNode->next->dst]) {  // ensure min value node appear only once
                minArr[curNode->next->dst]--;
            }
            else if (curNode->next->len > minArr[curNode->next->dst]) {
