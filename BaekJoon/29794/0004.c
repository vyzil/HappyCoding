...
int main(void){
    ...
    ll  minMovingTime, totalPotalTime, totalMovingTime;
    ...
    totalMovingTime = 0;
    for(i = 0 ; i < iM; i++){
        totalMovingTime += floorCount[i] * i;
    }
    ...
    for(s2 = s1+1; s2 < iM; s2++){
        ...
        for(i = 0; i < iM; i++){
            ...
            if(portalTime > i) portalTime = i;
            totalPotalTime += floorCount[i] * portalTime;
            if(minMovingTime < totalPotalTime) break;
        }
        if(minMovingTime >= totalPotalTime){
            res_s1 = s1;
            res_s2 = s2;
            res_st = totalMovingTime - totalPotalTime;
            minMovingTime = totalPotalTime;
        }
    }
