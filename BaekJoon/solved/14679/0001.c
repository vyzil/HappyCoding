...
int monAtkArr[ATK_MOD + 1];
int monDefArr[DEF_MOD + 1];
int monHpArr [ HP_MOD + 1];
...
void nextChallenger(int num){
    MON.atk = monAtkArr[MON.atk];
    MON.def = monDefArr[MON.def];
    MON.hp  = monHpArr [MON.hp ];
    return;
}

void calcMonArr(){
    int i;
    for(i = 1; i < ATK_MOD + 1; i++) monAtkArr[i] = (fastPow(i, pParam.atk, ATK_MOD) + aParam.atk) % ATK_MOD + 1;
    for(i = 1; i < DEF_MOD + 1; i++) monDefArr[i] = (fastPow(i, pParam.def, DEF_MOD) + aParam.def) % DEF_MOD + 1;
    for(i = 1; i < HP_MOD  + 1; i++) monHpArr [i] = (fastPow(i, pParam.hp,  HP_MOD)  + aParam.hp ) % HP_MOD  + 1;
