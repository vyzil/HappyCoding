...
int main(void){
    ...
    killTimeArr = (ll*)malloc(sizeof(ll)*bossNum);
    dmgHeap[0] = 0;
    for(i = 0; i < charNum; i++) {
        scanf("%lld", &iDmg);
        // insert Heap
    ...
    while(useNum--){        
        ...
        // make Kill Time Arr
        for(i = 0; i < bossNum; i++){
            killTimeArr[i] = KILL_TIME(bossArr[i].hp, curDmg);
        }
        ...
        for(i = 0; i < (1 << bossNum); i++){
            ...
            for(j = 0; curCase > 0 ; j++){
                if(curCase & 1) {
                    totalTime += killTimeArr[j];
                    curMeso += bossArr[j].meso;
                }
                if(totalTime > TIME_LIMIT) break;
                curCase >>= 1;
            }
            if(totalTime <= (ll)TIME_LIMIT && maxMeso < curMeso) maxMeso = curMeso;
        }
        totalMeso += maxMeso;
    }
    printf("%lld\n", totalMeso);
