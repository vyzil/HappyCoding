...
void solve(){
    ...
    for(i = 0; i < iN; i++){
        ...
        match = 1;
        for(j = 0; j < strSize; j++){
            if(encDict[atoi(decStr[j])] == 0) encDict[atoi(decStr[j])] = encStrList[i][j];
            else if(encDict[atoi(decStr[j])] != encStrList[i][j]) encDict[atoi(decStr[j])] = -1;
        }
    }
